		...
		struct pagetable_layout_info *pt_info)
{
	...
	ret.num_page = ((end >> pt_info->page_shift) -
			(start >> pt_info->pmd_shift <<
			 pt_info->pmd_shift >> pt_info->page_shift));
	ret.num_pmd  = ((end >> pt_info->pmd_shift) -
			(start >> pt_info->pud_shift <<
			 pt_info->pud_shift >> pt_info->pmd_shift));
	ret.num_pud  = ((end >> pt_info->pud_shift) -
			(start >> pt_info->p4d_shift <<
			 pt_info->p4d_shift >> pt_info->pud_shift));
	ret.num_p4d  = ((end >> pt_info->p4d_shift) -
			(start >> pt_info->p4d_shift));
	ret.num_pgd  = 512;
...
int main(int argc, char *argv[])
{
	...
	if (argc < 4) {
		if (argc == 2 && !strcmp(argv[1], "-h")) {
			...
			fprintf(stderr,
			"Usage: ./vm_inspector [-v] pid va_begin va_end\n");
			exit(-1);
		}
	} else if (argc == 4) {
	...
	page_table_addr = mmap(NULL, spaces.num_page * PAGE_SIZE,
	...
	fake_pmds = mmap(NULL, spaces.num_pmd * ADDR_SIZE,
			PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS,
			-1, 0);
	if (fake_pmds == (void *) -1) {
	...
	fake_puds = mmap(NULL, spaces.num_pud * ADDR_SIZE,
			PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS,
			-1, 0);
	if (fake_puds == (void *) -1) {
	...
	fake_p4ds = mmap(NULL, spaces.num_p4d * ADDR_SIZE,
			PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS,
			-1, 0);
	if (fake_p4ds == (void *) -1) {
	...
	fake_pgd = mmap(NULL, spaces.num_pgd * ADDR_SIZE,
			PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS,
			-1, 0);
	if (fake_pgd == (void *) -1) {
